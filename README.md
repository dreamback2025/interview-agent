...
**核心能力**：面试记录结构化落库 · 弱项分析 + 补强建议 · 知识库语义检索（标题切分 + 1024 维向量 + HNSW 索引）· **混合检索（向量 + 关键词 + RRF 融合）** · **拒答机制（相关性闸门）** · Function Calling（`@Tool` 由模型自主调用）· 多轮模拟面试与打分 · SSE 流式复盘 · 跨记录会话记忆 · 结果缓存与幂等 · 异步分析任务 · **JWT 鉴权与多用户数据隔离** · **按用户限流（滑动窗口 + Redis Lua）** · traceId 全链路 + LLM/RAG 指标 · 34 项一键自检
...
不是「知道自己的边界在哪」。真正的解法是**拒答机制**（置信度低时明确说「语料里没有相关内容」），而不是继续调阈值。
...
§2.16 证明了「该拒绝的拒绝不了」：正样本最低分 38.3%、难负样本最高 80.1%，重叠 41.8 个百分点。
阈值调不出来，只能换思路 —— **让系统知道自己不知道**。
...
规则：`vec < vecLow 且 cov < covLow` → 拒；`vec < vecMid 且 cov < covMid` → 拒；其余放行。

### ⚠️ 踩过的坑：第一个信号选错了

最初用的第二个信号是「关键词通道**命中文档数**」，实测**完全无效**：

```
Q: Kafka 消息积压该怎么处理（L1 跨域，本该拒绝）→ kwHits = 6
```

关键词通道是 **OR 查询**（`tok1 | tok2 | ...`），任意 token 命中就计数，而「消息」「处理」「怎么」
这类通用词在语料里到处都是。实测正负样本的命中数都是 6~15，**毫无区分度**。

换成**覆盖率**（要求 token 落在**同一个文档**里）才有区分力：

| 组 | n | vec 中位 | **cov 中位** | cov 区间 |
|---|---|---|---|---|
| **正样本** | 40 | 62% | **29%** | 11~88% |
| L1 跨域 | 19 | 53% | **12%** | 0~36% |
| L2 同域不同主题 | 7 | 57% | **17%** | 11~25% |
| L3 同主题边界 | 5 | 56% | **23%** | 15~38% |
| **L4 词法陷阱** | 3 | **73%** | **23%** | 17~45% |
...
阈值网格（`run-eval.py` 自动生成，用同一批信号本地模拟，不重调模型）：

| vecLow | vecMid | covLow | covMid | 漏答 | L1 拒 | L2~L4 拒 |
|---|---|---|---|---|---|---|
| **0.40** | **0.58** | **0.15** | **0.35** | **4/40** | **16/19** | **8/15** |
| 0.40 | 0.58 | 0.25 | 0.30 | 4/40 | 15/19 | 8/15 |
| 0.45 | 0.58 | 0.30 | 0.50 | 6/40 | 16/19 | 8/15 |

当前默认取第一行（同样拒绝力下漏答最少）。

### 为什么 L4 拒不掉（本轮最有价值的结论）

看上面的信号分布：**L4 的 vec 中位 73%，比正样本的 62% 还高**，cov 也不低（23%）。

因为 L4 是「**用词完全合法、只把语义翻转**」的陷阱题（「InnoDB 用 B 树就够了」）——
从检索的角度看，它跟正样本没有任何区别。**任何基于「像不像」的信号都区分不了它。**

> 拒答能解决「语料里没有这个主题」（L1，84%）、能解决一部分边界与同域问题（L2/L3，57~80%），
> **但解决不了「语料里有这个主题、只是说法被歪曲了」（L4）** —— 那需要语义判断
> （cross-encoder rerank，或先让 LLM 判断「检索到的内容支持这个说法吗」）。

这说明拒答与 rerank 是**互补而非替代**：拒答管「有没有」，rerank 管「对不对」。

### 评估口径：总体准确率会骗人

只看一个总数：**正确决策 60/74 = 81.1%**。但这个数字掩盖了关键信息 —— 两类错误的代价并不对称：

| 口径 | 数值 | 代价 |
|---|---|---|
| **误答率**（负样本被放行）| **10/34 = 29.4%** | **高**：用户被误导，而且答案看起来煞有介事 |
| 漏答率（正样本被拒）| 4/40 = 10.0% | 低：用户换个问法即可 |

误答的具体样子（`python3 scripts/eval/gate-errors.py` 可复现）：

| 用户问（语料里没有） | 系统却答 |
|---|---|
| MyBatis 的一级缓存和二级缓存有什么区别 | 「IOC 与循环依赖」|
| Elasticsearch 的查询性能怎么优化 | 「慢查询定位与 SQL 优化」|
| MySQL 按时间分区后跨分区查询怎么走 | 「分布式 ID 与分库分表」|

**每 3 个「语料里没有」的问题就有 1 个被硬答** —— 这在生产上不可接受，用户看到「问 MyBatis 答 IOC」会直接失去信任。

### ⚠️ 两个必须说明的局限

1. **统计不可靠**：n=74 太小，总体准确率的 95% 置信区间为 **70.7% ~ 88.4%（宽 18pp）**。
2. **阈值有过拟合风险**：`vecLow / vecMid / covLow / covMid` 是在**这 74 条上**网格搜出来的，
   换一批数据未必保持。要消除它需要用留出法（一半调参、一半评估）+ 更大的评测集。

> 因此当前定位是**辅助而非自动拦截**：把 `confident` 与 `signals` 暴露给前端，
> 提示「这条答案的把握度较低，请自行核对」，而不是替用户决定答或不答。
> 真正把误答率压下来要靠 rerank（见 §2.18 规划）。

### API

```bash
# 带相关性判定的检索（推荐）
curl "http://127.0.0.1:8092/api/knowledge/ask?q=xxx&topK=5" -H "Authorization: Bearer $TOKEN"
# → {"results":[…], "confident":false, "reason":"语料里没有找到相关内容（…）", "signals":{…}}

# 原接口保持不变（兼容既有调用方）
curl "http://127.0.0.1:8092/api/knowledge/search?q=xxx&topK=5"
```

### 配置与局限

```bash
RAG_GATE_ENABLED=false ./run.sh                        # 关掉闸门，退回 /search 行为
RAG_GATE_VEC_LOW=0.40 RAG_GATE_COV_MID=0.35 ./run.sh   # 调阈值（值由评测网格搜索得到）
```

局限：**闸门只能拒绝，不能正确回答**。L3 这类「语料讲了规则、没讲这个例外」的问题，
