...
**核心能力**：面试记录结构化落库 · 弱项分析 + 补强建议 · 知识库语义检索（标题切分 + 1024 维向量 + HNSW 索引）· **混合检索（向量 + 关键词 + RRF 融合）** · Function Calling（`@Tool` 由模型自主调用）· 多轮模拟面试与打分 · SSE 流式复盘 · 跨记录会话记忆 · 结果缓存与幂等 · 异步分析任务 · **JWT 鉴权与多用户数据隔离** · **按用户限流（滑动窗口 + Redis Lua）** · traceId 全链路 + LLM/RAG 指标 · 34 项一键自检
