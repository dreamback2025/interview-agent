...
| Hit@5 | 40/40 = 100.0% |
| **MRR** | **0.904** |
| Top1 相似度 | 均值 61.7%，区间 38.3%~78.3% |

以下 7 条查询的正确答案**未排在 Top1**（最后一列是它在结果中的实际排名，`>5` 表示未被召回）：

| 期望知识点 | 提问 | 实际 Top1 | 正确排名 |
|---|---|---|---|
| `jvm-gc` | CMS 当年为什么被废弃，它有什么硬伤 | ConcurrentHashMap 与并发容器 | 2 |
| `jvm-classload` | 为什么自己写一个 java.lang.String 不会生效 | 事务失效与传播行为 | 2 |
...
|---|---|---|---|
| **Hit@1** | 28/40 = 70.0% | 33/40 = **82.5%** | **+12.5pp** |
| Hit@3 | 95.0% | 100.0% | +5.0pp |
| Hit@5 | 95.0% | 100.0% | +5.0pp |
| **MRR** | 0.827 | **0.904** | +0.077 |
| 未命中 Top1 | 12 条 | 7 条 | -5 |
...
|---|---|---|---|---|
| **L1** | 跨域（语料无此技术栈） | 19 | 52.2% | **59.9%** |
| **L2** | 同域不同主题（域在语料内，主题没讲） | 7 | 52.7% | **57.9%** |
| **L3** | 同主题边界（讲了规则，没覆盖此例外） | 5 | 55.3% | **63.2%** |
| **L4** | 词法陷阱（用词与正样本重叠，语义翻转） | 3 | 67.5% | **80.1%** |
...
| L2 | 57.9% | Redis 的大 key 怎么治理，Pipeline 和事务有什么区别 | 缓存穿透、击穿、雪崩 |
...
| L4 | 63.2% | i++ 用 volatile 修饰就是线程安全的了 | volatile、CAS 与 ABA |

![负样本分级分布](rag-eval-distribution.svg)

正样本 Top1 最低分 **38.3%**，难负样本（L2~L4）最高分 **80.1%** —— 重叠 **41.8 个百分点**。
...
|---|---|---|---|
| ≥45% | 39/40 (97.5%) | 17/19 (89.5%) | **13/15 (86.7%)** |
| ≥50% | 38/40 (95.0%) | 12/19 (63.2%) | **13/15 (86.7%)** |
| ≥52% | 37/40 (92.5%) | 10/19 (52.6%) | **11/15 (73.3%)** |
| ≥54% | 36/40 (90.0%) | 7/19 (36.8%) | **10/15 (66.7%)** |
| ≥56% | 34/40 (85.0%) | 5/19 (26.3%) | **7/15 (46.7%)** |
| ≥58% | 30/40 (75.0%) | 3/19 (15.8%) | **4/15 (26.7%)** |
| ≥60% | 24/40 (60.0%) | 0/19 (0.0%) | **3/15 (20.0%)** |
| ≥65% | 8/40 (20.0%) | 0/19 (0.0%) | **1/15 (6.7%)** |
| ≥70% | 4/40 (10.0%) | 0/19 (0.0%) | **1/15 (6.7%)** |

## 5. 拒答策略评估

§3 / §4 证明了「该拒绝的拒绝不了」。于是加一层**相关性闸门**（`RagRelevanceGate`）：不只返回最像的片段，还要判断这些片段够不够回答问题，不够就返回 `confident=false`。

### 为什么不用单一相似度阈值

正样本最低分 38.3%、难负样本最高分 80.1%，**重叠 41.8 个百分点** —— 
单一阈值必然二选一地失败。改用**双信号**：
...
判定规则（阈值可配）：`vecTop1 < vecLow 且 coverage < covLow` → 拒；`vecTop1 < vecMid 且 coverage < covMid` → 拒。
...
**权衡逻辑**：漏答（把能答的问题拒了）的代价**高于**误答（答了语料里没有的内容）—— 用户被拒后可以换个问法，但拿到一个「看似相关实则应拒」的答案会被直接带偏。所以策略是「在漏答可控的前提下，尽量多拒难负样本」。

> 局限：闸门只能拒绝，不能**正确回答**。L3 这类「语料讲了规则、没讲这个例外」的问题，理想行为是回答「我讲了最左前缀，但没覆盖 8.0 的跳跃扫描」—— 这需要检索时带上「知识点覆盖范围」的元信息，属于后续工作。

## 6. 结论

1. **排序指标可用**：Hit@1 82.5%、Hit@3 100.0%、MRR 0.904，说明检索在「自然口语提问 → 知识点」这个任务上排序是有效的。
2. **绝对相似度不可作阈值**：L1 负样本最高分 59.9%，但 L2~L4（同域 / 边界 / 词法陷阱）最高分也到了 80.1%，比正样本最低分（38.3%）还高 41.8 个百分点 —— 区间完全重叠。要让**难**负样本误判归零，阈值须提到 81% 以上，此时正样本通过率只剩 0.0%。**不存在可用的工作点**，因此这类系统的验收标准只能是排序指标（Hit@K / MRR），而不是「相似度 > 0.7 才算命中」。
3. **L1 与 L2~L4 是两个不同的问题**：L1 分数明显更低（均值 52.2%），说明「问别的技术栈」系统是能识别的；但 L2~L4 抬升到 56.6%，与正样本区间重叠 ——**真正的风险不是「问别的技术栈」，而是「问同域里没讲的那个点」**。
4. **topK = 3 是性价比最高的取值**：7 条查询的正确答案不在 Top1，其中 7 条落在 Top2~Top3；Hit@3 与 Hit@5 相同（均 100.0%），说明取 3 条已覆盖全部可召回结果，取 5 条只是徒增送入 LLM 的上下文开销。
5. **提升方向**：剩余错误来自主题相近的知识点互相干扰 —— 已落地混合检索（向量 + 关键词 tsvector + RRF），Hit@1 从 70%% 提到 85%%；进一步可用 cross-encoder rerank 做语义级精排。

---

*评测耗时 3 秒；命中判定基于文档 tags 自动完成，无人工标注介入。*
